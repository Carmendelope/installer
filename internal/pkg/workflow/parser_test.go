...
var _ = ginkgo.Describe("Parser", func() {
	...
	ginkgo.Context("parses a workflow iterating through the nodes", func(){
		...
		ginkgo.It("must have iterated through the nodes", func(){
			...
			gomega.Expect(firstCmd.(*sync.Exec).Args[0], gomega.Equal(params.InstallRequest.InstallId))
			gomega.Expect(firstCmd.(*sync.Exec).Args[1], gomega.Equal(params.InstallRequest.ClusterId))
			for i := 0; i < numNodes; i++ {
				toCheck := (*workflow).Commands[i+1]
				gomega.Expect(toCheck.(*sync.Exec).Cmd, gomega.Equal(fmt.Sprintf("cmd%d", i)))
				gomega.Expect(toCheck.(*sync.Exec).Args[0], gomega.Equal(fmt.Sprintf("10.1.1.%d", i)))
			}
		})
	...
	ginkgo.Context("parses a simple workflow with two different commands", func(){
		...
		ginkgo.It("must be returned and contain the Exec and SCP commands", func(){
			...
			gomega.Expect(workflow, gomega.Not(gomega.BeNil()))
			cmd1 := (*workflow).Commands[0]
			gomega.Expect(cmd1.Name(), gomega.Equal(entities.Exec))
			gomega.Expect(cmd1.(*sync.Exec).Cmd, gomega.Equal("cmd1"))
			cmd2 := (*workflow).Commands[1]
			gomega.Expect(cmd2.Name(), gomega.Equal(entities.SCP))
			gomega.Expect(cmd2.(*sync.SCP).TargetHost, gomega.Equal("127.0.0.1"))
		})
	})
})
